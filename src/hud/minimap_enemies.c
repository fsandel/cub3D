...
{
	const double	scale = window->hud->minimap_radius
		/ window->hud->minimap_zoom;
	const int		x_pos = (enemy->pos.x - window->player->pos.x) * scale
		+ window->hud->minimap_pos_x;
	const int		y_pos = (enemy->pos.y - window->player->pos.y) * scale
		+ window->hud->minimap_pos_y;

	if (is_on_screen(x_pos, y_pos)
		&& quad_add(x_pos - window->hud->minimap_pos_x,
			y_pos - window->hud->minimap_pos_y)
		< window->hud->minimap_radius - 1)
