...
void	free_all_enemies(t_enemy **all_enemies)
{
	...
	if (all_enemies[0])
	{
		...
		{
			ft_mlx_delete_texture(all_enemies[0]->attacking_tex[tex_iter]);
			ft_mlx_delete_texture(all_enemies[0]->walking_tex[tex_iter]);
			tex_iter++;
		}
		ft_mlx_delete_texture(all_enemies[0]->dead_tex);
	}
	enemy_iter = 0;
