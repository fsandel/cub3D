				...
				int x, t_direction direction)
{
	...
	{
		if (y < start)
			put_pixel_floor(window->img, x, y,
				window->map->ceiling_color);
		else if (y >= start + line_height - 1)
			put_pixel_floor(window->img, x, y,
				window->map->floor_color);
		else
			mlx_put_pixel(window->img, x, y,
				get_rgba_from_tex_fog(texture,
				texture_x_value(texture, target, direction),
				texture_y_value(texture, line_height, y, start), dis));
		y++;
	}
}
				...
				double dis)
{
	...
	double		brightness = max(1.0 - (dis / FOG_RANGE), 0);
...
static void put_pixel_floor(mlx_image_t *img, int pix_pos_x, int pix_pos_y,
			int base_color)
{
	double brightness = abs(HEIGHT / 2 - pix_pos_y) / 900.0;
